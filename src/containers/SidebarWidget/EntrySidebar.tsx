...
const EntrySidebarExtension = () => {
  ...
  const handleFetchDraft = useCallback(async () => {
    const data = await appSdk?.location?.SidebarWidget?.entry.getDraftData();
    console.log("Fetched Draft Entry:", data);
    setDraftData(data);

    await appSdk?.location?.SidebarWidget?.entry.onChange((data) => {
